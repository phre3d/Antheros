 #monster #Mimic_Mayhem

# Tiny Mimic
tiny monstrosity, neutral

**Armour Class:** 10<br>
**Hit Points:** 5 <mark style="background:#d3f8b6">10</mark> <mark style="background:#fff88f">20</mark> <mark style="background:#ff4d4f">35</mark><br>
**Proficiency Bonus:** <br>
**Movement:** 15'

---

| **STR** | **DEX** | **CON** | **INT** | **WIS** | **CHA** |
|:-------:|:-------:|:-------:|:-------:|:-------:|:-------:|
|  13(+1) |  16(+2) |  13(+1) |  5(-3) |  11(+0) |  6(-2) |

---

**Senses:** Darkvision 60', passive perception 9<br>
**Languages:** n/a<br>
**Challenge:**  

**Damage Resistances:** <br>
**Damage Immunities:** acid<br>
**Condition Immunities:** prone<br>

>[!infobox]
> ![[Plot/Mimic Mayhem/Images/tkn.Rat Mimic (Tiny).png]]
>

---

# Special Abilities
**Shapechanger.** The mimic can use its action to polymorph into an object or back into its true, amorphous form. Its statistics are the same in each form. Equipment is not transformed. It reverts to its true form if it dies.

**False Appearance** (Object Form Only). While motionless, the mimic is indistinguishable from an ordinary object.

<mark style="background:#d3f8b6">**Adhesive Pseudopods.** The pseudopods adhere to anything they touch. A creature adhered to one is also grappled by it (escape DC 13). Ability checks made to escape this grapple have disadvantage.</mark>

<mark style="background:#d3f8b6">**Grappler.** The mimic has advantage on attack rolls against any creature grappled by it.</mark>

<mark style="background:#ff4d4f">**Magic Resistance.** The mimic has advantage on all saving throws against spells and magic.</mark>

<mark style="background:#ff4d4f">**Regeneration.** As one of their attacks, the mimic can swallow a pseudopod to instantly heal 5 hps. It regenerates a new pseudopod in 1d4 days.</mark>


---

# Actions
<mark style="background:#fff88f">**Multi-Attack.** The mimic can make two attacks. One with its pseudopod and one with its bite.</mark>

**Pseudopod.** <br>
*Melee Weapon Attack:* +3 to hit, <u>reach 5 ft.</u>, one target.  <br>
*Hit:* 3 (1d4 + 1) bludgeoning damage <mark style="background:#d3f8b6">and grappled. A Pseudopod that is already grappling someone does not attack again but instead tries to pull their victim ten feet closer by winning an opposed Athletics check.</mark>

**Bite.**  <br>
*Melee Weapon Attack:* +3 to hit, <u>reach 5 ft.</u>, one target.  <br>
*Hit:* 3 (1d4 + 1) piercing damage plus 2 (1d4) acid damage.

---

| Combined Party Level | Adjustment |
|:---------------:|------------|
|12 or less | Ignore all the colored special abilities.|
|13 to 30 | Add only the green special abilities.|
|31 to 48 | Add the green and yellow special abilities.|
|49 to 78 | Use all the colored special abilities |

